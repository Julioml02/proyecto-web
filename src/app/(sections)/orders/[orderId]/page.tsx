...
import { userOrderItem, getOrder} from '@/lib/handlers';
...
export default async function Order({
  ...
  const orders = await userOrderItem(session.userId, params.orderId);
      ...
      <table className="w-full max-w-md">
        <tbody>
          ...
          <tr>
            ...
            <td className="pb-2 text-sm sm:text-base md:text-lg lg:text-xl" style={{whiteSpace: 'nowrap'}}>
              { orders.cardNumber +' (' + orders.cardHolder +')'}
            </td>
          </tr>
          ...
          <tr>
            ...
            <td className="pb-2 text-sm sm:text-base md:text-lg lg:text-xl" style={{whiteSpace: 'nowrap'}}>
              { 
              new Date(orders.date).toLocaleString('en-US', 
                { 
                  year: 'numeric', 
                  month: 'long', 
                  day: 'numeric', 
          ...
          {orders?.orderItems.map((orderItem) => (
            <tr key={orderItem.product._id.toString()} className=' bg-white '>
              ...
              <td className='p-4 text-xs sm:text-sm md:text-base lg:text-lg w-20/100 hidden sm:table-cell md:hidden lg:table-cell text-center'>
                {
                  orderItem.price.toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })
                }
              </td>
